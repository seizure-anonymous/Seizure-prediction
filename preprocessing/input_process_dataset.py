...
class ReadTFRecords(object):
    def __init__(self, ordered_signals, batch_size, train_files, eval_files, test_files, patient_specific='00',
    ...
    def tfrecords_to_batch(self, tfrecords_list, name):
        dataset = tf.data.Dataset.from_tensor_slices(tfrecords_list). \
            interleave(lambda x: tf.data.TFRecordDataset(x).map(self.parse_fn).repeat(),
                       cycle_length=len(tfrecords_list))
        dataset = dataset.shuffle(self.min_queue_examples).batch(self.batch_size)
        iterator = dataset.make_one_shot_iterator()
        next_element = iterator.get_next()
        return {"signals": next_element[0], "simple_signals": next_element[1], "labels": next_element[2]}
...
class ReadTFRecords_data(object):
    def __init__(self, ordered_signals, batch_size, train_files, eval_files, test_files, patient_specific='00',
                 min_queue_examples=128):
        self.signals = ordered_signals
        self.num_files = params.seizure_samples[params.input_length]
    ...
    def dataset_build(self):
        ...
        for label in self.labels:
            ...
            for file in self.test_files:
                add_dir_in_list(test_dir_list, self.dataset_dir + "/%s/%s.tfrecords" % (label, file))
        for dir_list in [train_dir_list, eval_dir_list, test_dir_list]:
            filename_queue = tf.train.string_input_producer(dir_list)
            reader = tf.TFRecordReader(name=None, options=None)
            _, serialized_example = reader.read(filename_queue)
            len_channel=len(params.normal_signal)
            feature_tfrecords={"label": tf.FixedLenFeature([], tf.int64)}
            for i in range(len_channel):
                feature_tfrecords[params.normal_signal[i]] = tf.FixedLenFeature([], tf.int64)

            self.features = tf.parse_single_example(
                serialized_example,
                features=feature_tfrecords
            )
    def augmentation(self, signals, simple_signals, labels):
        return signals, simple_signals, labels
...
def test_input():
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=False, log_device_placement=False)) as sess:
        ...
        for step in range(50):
            ...
            # f.close()
        avg_duration = all_duration / 50
        print("tf.data run 500 steps with average duration: %.3fs" % avg_duration)
        avg_duration = (time.clock() - all_duration_start) / 50
        print("tf.data run 500 steps with average duration: %.3fs" % avg_duration)
