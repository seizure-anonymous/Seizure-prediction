...
global datapath
datapath='/data/zhengruifeng/zhengruifeng/chb-mit-scalp-eeg-database-1.0.0/major_revison_post_pre_ictal_1h_len_2/pred_v2/split/'
# datapath="/data/zhengruifeng/zhengruifeng/chb-mit-scalp-eeg-database-1.0.0/all24_preictal_1h_post_pre_ictal_1h_len_2/pred_v2/split/"
...
#"/home/zhongying/interictaldata/zheer_preictal_30m_post_pre_ictal_1h_len_2/pred_v2/split/"
#"/data/zhengruifeng/zhengruifeng/chb-mit-scalp-eeg-database-1.0.0/77_preictal_1h_post_pre_ictal_4h_len_2/pred_v2/split/"
...
def train(args, io):
    ...
    elif args.model == 'All_Transformer_self':
        channel_sizes = [8, 4, 4]
        kernel_size = 4
        model = network_multi.All_Transformer_self(44, 2, channel_sizes, kernel_size=kernel_size, dropout=0.6,
                                                   channel=args.channel).to(device)
    elif args.model == 'LSTM_Transformer':
        channel_sizes = [8,4,4]
        kernel_size = 4
        model = network_multi.LSTM_Transformer(44, 2, channel_sizes, kernel_size=kernel_size, dropout=0.6, channel=args.channel).to(device)
    elif args.model == 'CNN_Transformer':
        channel_sizes = [8,4,4]
        kernel_size = 4
        model = network_multi.CNN_Transformer(44, 2, channel_sizes, kernel_size=kernel_size, dropout=0.6, channel=args.channel).to(device)
    ...
    # datapath = "/dataset/zhengruifeng/chb-mit-scalp-eeg-database-1.0.0/preictal_30m_post_pre_ictal_1h_len_2/pred_v2/split/"
    # patient = '05'
    seizure_list=os.listdir(os.path.join(datapath,args.patient))
    seizure_time=seizure_list.__len__()
    # dataset_train, dataset_val=CHB_data.dataloader_train(datapath=datapath, partition='train', patient=args.patient, seizure_time=seizure_time, seizure_for_test=args.seizure)

    # seizure_for_val=2
    # seizure_for_test=2
    dataset_train = CHB_data.CHB_MIT_LOOCV_pred_not_event \
        (datapath=datapath, partition='train', patient=args.patient, seizure_time=seizure_time,
         seizure_for_val=args.seizure_val, seizure_for_test=args.seizure_test)
    dataset_val = CHB_data.CHB_MIT_LOOCV_pred_not_event \
        (datapath=datapath, partition='val', patient=args.patient, seizure_time=seizure_time,
         seizure_for_val=args.seizure_val, seizure_for_test=args.seizure_test)
    # dataset_test = CHB_data.CHB_MIT_LOOCV_pred_not_event \
    #     (datapath=datapath, partition='test', patient=args.patient, seizure_time=seizure_time,
    #      seizure_for_val=args.seizure_val, seizure_for_test=args.seizure_test)

    # dataset_train=CHB_data.CHB_MIT_LOOCV_specific_pred\
    #     (datapath=datapath, partition='train', patient=args.patient, seizure_time=seizure_time, seizure_for_val=args.seizure_val, seizure_for_test=args.seizure_test)
    # dataset_val=CHB_data.CHB_MIT_LOOCV_specific_pred\
    #     (datapath=datapath, partition='val', patient=args.patient, seizure_time=seizure_time, seizure_for_val=args.seizure_val, seizure_for_test=args.seizure_test)
    # dataset_test=CHB_data.CHB_MIT_LOOCV_specific_pred\
    #     (datapath=datapath, partition='test', patient=args.patient, seizure_time=seizure_time, seizure_for_val=args.seizure_val, seizure_for_test=args.seizure_test)
    ...
    def train_after(best_test_acc, loader, type):
        ...
        for data, label in tqdm(loader, desc=type):
            if args.channel != 'empty':
                ...
                channel = [int(item) for item in channel]
            else:
                channel = []
            if len(channel) >= 1:
                data = data[:, :, channel, :]
            ...
            # loss=multi_cal.loss
            loss_after = loss_compute(logits, label)
            preds = logits.max(dim=1)[1]
            count += batch_size
            test_loss += loss_after.item() * batch_size
            try:
        ...
        if args.model == 'All_Transformer' or True:
            ...
            io.cprint(str((channel_aware.indices)))
            io.cprint(str(channel_aware.values))
        ...
        if type=='val':
            if test_acc >= best_test_acc:
                ...
                if best_test_acc>0:
                    torch.save(model.state_dict(), 'checkpoints/%s/models/model_%.4f_%s.t7' % (args.exp_name, best_test_acc, args.channel))
            outstr = '%s %d, loss: %.6f, test acc: %.6f, test avg acc: %.6f, best_test: %.6f' % (type, epoch,
                                                                                                 test_loss * 1.0 / (count+0.01),
                                                                                                 test_acc,
                                                                                                 avg_per_class_acc,
                                                                                                 best_test_acc)
            io.cprint(outstr)
    ...
    for epoch in range(args.epochs):
        ...
        best_test_acc_new=train_after(best_test_acc, val_loader, 'val')
        ...
        # if args.seizure_test!=None:#args.seizure_val!=None and args.seizure_val!=args.seizure_test:
        #     test_acc_loocv=train_after(0.0, test_loader, 'test')
        #     outstr1='acc_loocv: %f'%test_acc_loocv
        #     print(outstr1)
        #     io.cprint(outstr1)
        ####################
        # Test
...
def test(args, io):
    ...
    save_models=os.listdir('checkpoints/%s/models/' % (args.exp_name))
    model_list=[]
    length = len(args.channel)
    for save_model in save_models:
        if save_model[-3-length:-3]==args.channel:
            model_list.append(save_model)
    acc=[float(i[6:12]) for i in model_list]
    index=acc.index(max(acc))

    model.load_state_dict(torch.load('checkpoints/%s/models/%s' % (args.exp_name, model_list[index])))
    print("Let's use", torch.cuda.device_count(), "GPUs!")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--epochs', default=30, help='the train epoch')
