...
from train.loss_compute_torch import loss_compute, cal_loss
...
def train(args, io):
    ...
    if args.model == 'GcnNet':
        ...
        model = net_gcn.SetTransformer(args).to(device)
    elif args.model == 'Gcn_Transformer':
        model = net_gcn.Gcn_Transformer(args).to(device)
    elif args.model == 'Gcn_Transformer_time':
        model = net_gcn.Gcn_Transformer_time(args).to(device)
    ...
    if args.independent=='True':
        ...
        eval_loader = DataLoader(CHB_data.CHB_MIT_no_overlap(partition='eval'), num_workers=8,
                                 batch_size=args.batch_size, shuffle=False, drop_last=False)
    elif(args.loocv=='True'):
        train_loader = DataLoader(CHB_data.CHB_MIT_LOOCV_specific_pred(partition='train', patient='13', seizure_time=7, seizure_for_test=0), num_workers=8,
                                  batch_size=args.batch_size, shuffle=True, drop_last=True)
        test_loader = DataLoader(CHB_data.CHB_MIT_LOOCV_specific_pred(partition='test', patient='13', seizure_time=7, seizure_for_test=0), num_workers=8,
                                 batch_size=args.batch_size, shuffle=False, drop_last=False)
    else:
    ...
    def train_after(best_test_acc, loader, type):
        ...
        for data, label in tqdm(loader, desc=type):
            ...
            preds = logits.max(dim=1)[1]#.max(dim=1)[1]
            count += batch_size
